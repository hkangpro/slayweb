...
**심볼은 쓰지 않습니다.** 이름 자체가 마크입니다.

Relai는 Newsreader Light로 쓰고 **마지막 두 글자 `ai`만 기울여 초록**으로 뺍니다. 어두운 면에서는 그 초록을 `#6fd9b4`로 올립니다. 한 색만 쓸 수 있을 때는 기울임만 남깁니다. 파비콘은 그 기울인 `a` 한 글자입니다.

Blueanchor는 `PROCON`을 크게, `BLUEANCHOR`를 그 아래 주황으로 두 줄 락업입니다. 파비콘은 `BA`입니다.
